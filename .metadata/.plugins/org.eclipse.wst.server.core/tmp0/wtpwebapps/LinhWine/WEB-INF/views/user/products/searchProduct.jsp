			...
			<div class="col large-9">
				<div class="shop-container">
					...
					<div
						...
						<c:if test="${searchProductsPage.size() == 0 }">
							<h2 class="text-center">Không có sản phẩm cần tìm.</h2>
						</c:if>

						<c:forEach var="item" items="${searchProductsPage }">
						...
						<nav class="woocommerce-pagination">
							<div class="pagination-container">
								<ul class="text-center links nav-pagination page-numbers">
									...
									<c:forEach var="item" begin="1" end="${searchProductInfo.totalPage }" varStatus="loop">
										
										<c:if test="${(loop.index) == searchProductInfo.currentPage }">
											<li class="page-number active"><a href="<c:url value="/tim-kiem-san-pham/${loop.index }" />">${(loop.index)}</a></li>
										</c:if>
										<c:if test="${(loop.index) != searchProductInfo.currentPage }">
											<li class="page-number"><a href="<c:url value="/tim-kiem-san-pham/${loop.index }" />">${(loop.index)}</a></li>
										</c:if>

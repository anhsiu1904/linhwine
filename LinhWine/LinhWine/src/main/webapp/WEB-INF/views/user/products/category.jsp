		...
		<div class="shop-page-title category-page-title page-title ">
			<div class="page-title-inner flex-row  medium-flex-wrap container">
						...
						<nav class="woocommerce-breadcrumb breadcrumbs ">
							...
							<c:forEach var="item" items="${categories }">
								...
								<c:if test="${ item.id == idCategory }">
									...
									<a href='<c:url value="/" />'>Trang chủ</a> <span class="divider">/</span><span
										class="categorycustom"><firstletter>${ item.name} </firstletter></span>
				...
				<div class="flex-col medium-text-center">
						...
						<select  class="orderby" data-id="${idCategory }" >
							<option value="menu_order">Thứ
								tự mặc định</option>
							<option  value="tang">Theo giảm giá: thấp đến
								cao</option>
							<option  value="giam">Theo giảm giá: cao xuống
								thấp</option>
						</select>
		...
		<script>
	           ...
	           $(".orderby").change(function() {
	        	  var id = $(this).data("id");
	        	   ...
	        	   window.location =  "/LinhWine/san-pham/"+id+"/"+status+"/1";
	           		});
			...
			<div class="col large-9">
				<div class="shop-container">
					...
					<div
						...
						<c:if test="${ProductsPage.size() == 0 }">
						
							<h2 class="text-center">Không có sản phẩm</h2>
						
						</c:if>
						<c:if test="${not empty sortByIdPage }">
							<c:set var="list" value="${sortByIdPage }" />
							<c:set var="type" value="${type }" />
							
						</c:if>
						
						<c:if test="${empty sortByIdPage }">
						...
						<nav class="woocommerce-pagination">
							<div class="pagination-container">
								<ul class="text-center links nav-pagination page-numbers">
									<c:if test="${empty sortByIdPage }">
										<c:forEach var="item" begin="1" end="${pageInfo.totalPage }" varStatus="loop">
											...
											<c:if test="${(loop.index) == pageInfo.currentPage }">
												<li class="page-number active"><a href="<c:url value="/san-pham/${idCategory }/${loop.index }" />">${(loop.index)}</a></li>
											</c:if>
											<c:if test="${(loop.index) != pageInfo.currentPage }">
												<li class="page-number"><a href="<c:url value="/san-pham/${idCategory }/${loop.index }" />">${(loop.index)}</a></li>
											</c:if>
									...
									<c:if test="${not empty sortByIdPage }">
										<c:forEach var="item" begin="1" end="${sortProductByIdInfo.totalPage }" varStatus="loop">
											
											<c:if test="${(loop.index) == sortProductByIdInfo.currentPage }">
												<li class="page-number active"><a href="<c:url value="/san-pham/${idCategory }/${type}/${loop.index }" />">${(loop.index)}</a></li>
											</c:if>
											<c:if test="${(loop.index) != sortProductByIdInfo.currentPage }">
												<li class="page-number"><a href="<c:url value="/san-pham/${idCategory }/${type}/${loop.index }" />">${(loop.index)}</a></li>
											</c:if>
